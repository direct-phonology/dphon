...
class SmithWatermanAligner(Aligner):
    ...
    def __call__(self, match: Match) -> Match:
        ...
        while (not au[-1].isalnum() and au[-1] != self.gap_char) or (
            not av[-1].isalnum() and av[-1] != self.gap_char
        ):
            utxt, vtxt, au, av = utxt[:-1], vtxt[:-1], au[:-1], av[:-1]
        while (not au[0].isalnum() and au[0] != self.gap_char) or (
            not av[0].isalnum() and av[0] != self.gap_char
        ):
            utxt, vtxt, au, av = utxt[1:], vtxt[1:], au[1:], av[1:]
